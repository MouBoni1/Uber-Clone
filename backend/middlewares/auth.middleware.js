...
module.exports.authUser = async (req, res, next) => {
    ...
    try {
        ...
        const user = await blacklistTokenModel.findById(decoded._id); // Find the user by ID
        if (!user) {
...
module.exports.authCaptain = async (req, res, next) => {
    ...
    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET); // Verify the token
        const captain = await captainModel.findById(decoded._id); // Find the user by ID
        if (!captain) {
            return res.status(401).json({ message: 'Unauthorized' }); // If user is not found
        }
